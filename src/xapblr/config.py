...
class Config:
    def __init__(self):
        ...
        try:
            ...
            print(f"Could not read API key from {key_file}: {e}", file=stderr)
        except JSONDecodeError as e:
            # Don't need to exit, we could get a valid config from config.json
            print(f"Malformed JSON in {key_file}: {e}", file=stderr)
        ...
        try:
            ...
            print(f"Could not read config from {key_file}: {e}", file=stderr)
        except JSONDecodeError as e:
            exit(f"Malformed JSON in {config_file}: {e}")
